...
import { uploadRegistered } from '@/services/importService';
...
const processData = async (results) => {
  ...
  try {
    ...
    let res;
    if (collectiveId.value.length > 0) {
      res = await uploadRegistered(cleanedData, collectiveId.value);
    } else {
       res = await uploadRegistered(cleanedData);
    }
...
<template>
  <div class="importer">
    <h3>Import Registered Members</h3>
    <input v-model="collectiveId" placeholder="collectiveId" type="text" />
    <input
      type="file"
      accept=".csv"
      ref="fileInput"
